...
@task(help={"version": "dlite-cuds version to set"})
def setver(_, version=""):
    """Sets the dlite-cuds version."""
    match = re.fullmatch(
        (
    ...
    update_file(
        TOP_DIR / "dlite_cuds" / "__init__.py",
...
def create_api_reference_docs(context, pre_clean=False, pre_commit=False):
    ...
    package_dir = TOP_DIR / "dlite_cuds"
...
def create_docs_index(_):
    ...
    replacement_mapping = [
        ...
        (
            "(docker-compose.yml)",
            "(https://github.com/EMMC-ASBL/dlite-cuds/blob/main/docker-compose.yml)",  # pylint: disable=line-too-long
        ),
        (
            "(setup.cfg)",
            "(https://github.com/EMMC-ASBL/dlite-cuds/blob/main/setup.cfg)",  # pylint: disable=line-too-long
        ),
    ]
